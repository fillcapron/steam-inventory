...
const SelectedGame: React.FC = () => {
    ...
    return (
        <div className="selectedGame">
            ...
            <div className="selectedGame-name"><h1>Инвентарь {appName[game]}</h1><p>Всего предметов: <span>{total_items ? total_items : 0}</span></p><p><span>{total_price ? 'Стоимость: ' + total_price + ' руб.' : ''}</span></p></div>
        </div>
    )
}
